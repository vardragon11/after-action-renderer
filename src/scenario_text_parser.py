from aar_models import *
from aar_tokenizer import *
import re
import json
...
def parse_event(chunk: str) -> BattleEvent:
    ...
    return BattleEvent(
        ...
        event_dest=(float(re.findall(battle_x,chunk)[0].replace(" ","")), float(re.findall(battle_y,chunk)[0].replace(" ",""))),
    )
...
def writeJSON():
    # Write to file
    with open("space_scenario.json", "w") as f:
